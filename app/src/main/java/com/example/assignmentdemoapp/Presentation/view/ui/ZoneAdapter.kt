...
import com.example.assignmentdemoapp.Domain.domain.entity.ZoneE
import com.example.assignmentdemoapp.R

class ZoneAdapter(
val items: List<ZoneE>
) : RecyclerView.Adapter<ZoneAdapter.MyViewHolder>() {
    var items_list = items
    var callback: RecyclerviewCallbacks<ZoneE>? = null

    fun setOnClick(click: RecyclerviewCallbacks<ZoneE>){
        callback = click
    }
...
//        var alert_filter_selected: ImageView = itemView.findViewById(R.id.alert_filter_selected)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        holder.tvName.text = items_list.get(position).zone
    }
}